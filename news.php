...
<html>
	...
	<body>
		...
		<div id="nav">
			...
			<div id="news" style="border-bottom:4px solid yellow;">News</div>
		...
		<h1 style="font-family:Calibri; text-align:center; margin-top:40px; margin-bottom:2px;">NEWS</h1><hr style="color:black; width:65px; margin-top:-1px;"></hr>
		
		
		<!--PHP for news display -->
		<?php
		...
		<div id="new">
			<h1 style="font-family:Calibri; text-align:center; margin-top:40px; margin-bottom:2px;">SHARE THE NEWS</h1><hr style="color:black; width:200px; margin-top:-1px;"></hr>
			<form method="POST" name="new" enctype="multipart/form-data">
				...
				<textarea type="text" id="newsText" placeholder="Enter Latest Notice and News" name="newsText"></textarea>
				...
				<?php
					...
					if(isset($_POST['newsButton']))
					{
						...
						if($_SESSION['login']=='1'){
							...
							if($db_con)
							{
								if(is_uploaded_file($file1)){$insert="INSERT INTO news(Topic,Text,File,Sem) VALUES('$topic','$text','$file2','$sem')";}
								else{$insert="INSERT INTO news(Topic,Text,Sem) VALUES('$topic','$text','$sem')";}
								$res=mysql_query($insert);
				...
				<?php 
					...
					function dele($x)
					{
						...
						if($db_con)
							{
								$del=mysql_query("SELECT File FROM news WHERE Topic='".$x."'");
								$element= mysql_fetch_assoc($del);
								$path=$element['File'];
								if($path!=null)
								unlink($path);
								$deles=mysql_query("DELETE FROM news WHERE Topic='".$x."'");
